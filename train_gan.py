...
def main(max_steps=222222, dataset="SCR", dry_run=False, gpu=True):
    ...
    with open(f'./AMP/cfg/debug.yaml') as f:
        cfg: dict = yaml.load(f, Loader=yaml.FullLoader)

    namestring = "TransformerL_V{}".format(version)
    wandb.init(project="test" if cfg['debug'] else "navdream", entity="alan_lanfeng", name = namestring)
    ...
    train_dataset = N3DWorldModelDataset(
        ...
        regen=dataset,
        lidar_mode="images",file_limit=10
    )
    if dry_run:
    ...
    for epoch in range(max_epochs):
        ...
        for it, (x, a, y, x_rs, y_rs, dones) in pbar:
            ...
            with torch.set_grad_enabled(is_train):
                ...
                real_sequence = copy.copy(y)
                fake_sequence = copy.copy(y_pred)

                disc_output = disc.model(fake_sequence)
                disc_loss = torch.mean((0.5 * torch.sum(torch.square(disc_output - 1), axis=-1)))
                ...
                real_sequence.requires_grad = True
                loss_disc, info = disc.update_disc(real_sequence, fake_sequence.detach())
                wandb.log(info)
