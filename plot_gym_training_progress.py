...
def plot_training_progress(logdirs, scenario=None, x_axis="total_steps", y_axis="reward"):
    ...
    plt.clf()
    fig, axes = plt.subplots(1, len(all_scenarios), num="scenario rewards")
    axes = np.array(axes).reshape((-1,))
    axes = {scenario: ax for ax, scenario in zip(axes, all_scenarios)}
...
def main(logdir="~/navrep3d",
         ...
         y_axis: str_enum(["reward", "difficulty", "progress"]) = "difficulty", # noqa (flake8 bug?)
